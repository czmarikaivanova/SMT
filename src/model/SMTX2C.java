...
public class SMTX2C extends SMTX1VI {

	public SMTX2C(Graph graph, boolean isLP, boolean includeC) {
	...
	protected void initVars() {
		try {
			...
			w = new IloNumVar[n][n][d][d];		
			for (int i = 0; i < n; i++) {
				for (int j = 0; j < n; j++) {
					for (int k = 0; k < d; k++) {
						for (int l = 0; l < k; l++) {
							w[i][j][l][k] = cplex.numVar(0,1);
						}
					}	
				}					
			}
				...
				for (int t = 0; t < d; t++) {
					for (int i = 0; i < n; i++) {
						for (int j = 0; j < n; j++) {
							if (j != i && s != t) {
								if (s < t) {
									cplex.addEq(cplex.sum(f[i][j][s][t], w[i][j][s][t]), x[i][j][s]);									
								}
								else {
									cplex.addEq(cplex.sum(f[i][j][s][t], w[i][j][t][s]), x[i][j][s]);
								}
